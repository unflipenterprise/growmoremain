    ...
    <pane>
      ...
      <pane-body>
          ...
          <div class="q-px-md q-pt-md">
              <q-input outlined v-model="name" type="text" class="input-style" label="Your Full Name" :rules="[ val => val && val.length > 0 || 'Enter your Name']" />
       ...
       ...mapActions("tenantDetailsModules", ["addAddressData"]),
        onSubmit () {
            ...
            this.$refs.myFormAddress.validate()
                .then(success => {
                    if (this.name!='' && this.phone!='' && this.email!='' && this.houseno!='' && this.streetadress!='' && this.town!='' && this.landmark!='' && this.city!='' &&  this.pincode!=''){
                        var dataList={
                            'name':this.name,
                            'phone':this.phone,
                            'email':this.email,
